...
2. **resume 必须指定 subagent_type**（如 `"dg-frontend-vue-dev"`）
3. **每批开发轮次结束后，DEV_ID 失效**，新批重新启动开发Agent
4. **同批修正循环中复用同一个 DEV_ID**，禁止启动新Agent
5. **同批修正循环中复用测试Agent ID**（TEST_COMPONENT_ID / TEST_LOGIC_ID / TEST_STYLE_ID），新批开发时重新启动
...
Agent(
  ...
  prompt: "需求文件路径：{REQUIREMENT_FILE}\n项目根目录：{PROJECT_ROOT}\n\n请阅读需求文档，产出 dev-plan.md、design-guide.md，并搭建项目基础设施（Vite + Vue 3 + TS + Pinia + Vue Router）。完成后只返回文件路径列表。"
)
```
...
11. **需求文件只传路径不读内容** — 初始化时只记录 `REQUIREMENT_FILE` 路径，把路径传给 dg-vue-planner 让它自己读
