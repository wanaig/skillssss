...
写入方式：用 Bash 执行：
```bash
jq '.agents.test_dataflow.id = "{YOUR_AGENT_ID}" | .agents.test_dataflow.updated = "{CURRENT_TIME}"' {OUTPUT_DIR}/../agent-registry.json > tmp.json && mv tmp.json {OUTPUT_DIR}/../agent-registry.json
```
