...
5. **错误穿透检查**：读取前端的全局错误处理（如 axios interceptors 或 error handler）和后端的全局错误处理中间件，确认后端返回的错误码/信息能正确穿透到前端并以用户可理解的方式展示
...
|------|------|------------------------|---------|
| **blocker** | 阻断 | CORS配置缺失导致跨域请求全部失败、Vite代理目标地址与后端端口不一致、认证中间件未注册导致所有请求401、环境变量关键字段缺失（API_BASE_URL等） | 三轮后仍存在则必须人工介入 |
| **major** | 主要 | CORS白名单遗漏前端开发地址、401错误未在前端统一拦截跳转登录、后端错误码穿透到前端后未做用户友好转换、前端环境变量与后端配置数据库端口不一致 | 三轮后仍存在则向用户报告 |
| **minor** | 轻微 | 请求拦截器中错误日志级别不当、Vite代理rewrite规则可优化、开发/生产环境配置差异化建议 | 三轮后允许标记为低质量通过 ⚠️ |

### 6. 判定规则

- **FAIL 条件**（任一满足即 FAIL）：
  - 后端未配置 CORS 中间件
  - CORS 允许的源不包含前端地址（开发环境可为 `*` 或 localhost 端口）
  - CORS 允许的方法不包含 OPTIONS（预检请求会失败）
  - CORS 允许的头不包含 Authorization（前端无法发送带 Token 的请求）
...
除了写入 markdown 报告文件，必须同时写入 JSON 格式的测试报告到 `{输出目录}/{模块名}-integration-report.json`：
...
**Agent ID 写入**：
完成测试并写入报告后，将你的 Agent ID 写入独立文件 `{项目根目录}/agent-registry/fullstack_integration.json`（避免多Agent并发写入同一文件导致ID丢失）。
...
echo '{"id":"YOUR_AGENT_ID","type":"fs-tester-integration","updated":"CURRENT_TIME"}' > {项目根目录}/agent-registry/fullstack_integration.json
```
...
注意：如果同一个后端服务的 CORS/Auth 配置对多个模块是共享的，可以在报告中简要提及"参见 module01 共享后端配置，CORS/Auth 部分结论同 module01-integration.md"。
