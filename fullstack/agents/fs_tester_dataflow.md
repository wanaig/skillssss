...
- 测试报告输出目录 `{FRONTEND_ROOT}/outputs/fs_tester_dataflow/`

### Step 2: 必读文件（按顺序）

1. **integration-design-guide.md** 中目标模块的 "接口映射""数据转换要求""错误处理映射" 部分
2. **前端 Store 文件**（`{FRONTEND_ROOT}/project/src/stores/{module}.ts`）：了解状态管理逻辑
3. **前端 API 调用文件**（`{FRONTEND_ROOT}/project/src/api/{module}.ts`）
4. **前端页面/组件文件**（`{FRONTEND_ROOT}/project/src/views/` 中相关的 .vue 文件）：了解 UI 如何使用 store 数据
...
| 检查项 | 说明 | PASS条件 |
|--------|------|---------|
| 触发点 | 哪些用户操作会触达这个 API | 组件中能追踪到明确的调用链 |
...
| 分页状态 | 当前页码、总条数是否存于 store | page, total 等 ref 存在 |
| 累计/替换 | 加载更多 vs 翻页的行为 | 翻页用替换，加载更多用 push(...) |
| totalPages 计算 | 前端是否有独立计算总页数 | 使用后端返回的 totalPages 或自行计算 |

### 测试执行方法（如何审查，而非审查什么）

你通过**状态转换追踪**完成数据流测试，不运行服务器、不发送 API 请求。具体操作步骤：

1. **绘制数据流路径**：从用户触发点（按钮点击/onMounted/路由进入）开始，追踪数据从 API 调用 → store 更新 → 组件渲染的完整路径
2. **验证三态覆盖**：对每个 API 调用点，向前搜索 loading 状态的设置（如 `isLoading = true`），向后搜索 data（成功时的状态更新）和 error（失败时的 catch 处理），确认三态分支均存在
3. **检查状态一致性**：搜索 `localStorage`/`sessionStorage` 读写，确认持久化状态的键名与前后端约定一致；检查 store 的多个 action 之间是否有状态覆盖风险
4. **Token 流转验证**：搜索 `token`/`Authorization` 关键词，确认 Token 从登录获取 → 存储 → 每次请求携带 → 过期刷新 → 登出清除的完整链路
5. **数据转换检查**：对比后端 API 返回的原始数据与前端使用的数据结构，确认转换层（如 snake_case → camelCase）存在且正确
...
为每个模块输出一份报告到 `{FRONTEND_ROOT}/outputs/fs_tester_dataflow/{模块名}-dataflow.md`：
...
|------|------|------------------------|---------|
| **blocker** | 阻断 | loading状态未设置导致UI假死、error状态缺失导致请求失败后页面无反馈、Token刷新失败后未清除旧Token导致死循环、状态更新覆盖导致数据丢失 | 三轮后仍存在则必须人工介入 |
| **major** | 主要 | data三态（loading/error/empty）覆盖不全、Token过期后未自动重定向登录页、localStorage持久化键名与后端约定不一致、多个action同时修改同一state存在覆盖风险 | 三轮后仍存在则向用户报告 |
| **minor** | 轻微 | snake_case转camelCase转换可抽取为公共工具、Store中重复的状态计算可合并为computed、API请求可合并为批量请求 | 三轮后允许标记为低质量通过 ⚠️ |
...
除了写入 markdown 报告文件，必须同时写入 JSON 格式的测试报告到 `{输出目录}/{模块名}-dataflow-report.json`：
