...
Harness Engineering 系统的完整使用流程，从架构设计到前后端并行开发、联调、部署上线的分步操作指南，包含参数说明和日志体系。

## When to Use This Skill

- 需要按步骤执行多智能体协同开发流程时
- 需要了解每个阶段的启动命令和所需参数
- 需要配置 BATCH_SIZE 或理解日志文件结构
- 需要启动 Flutter 跨端或区块链智能合约开发
...
?? **必须等 frontend/ 和 backend/ 全部完成后再执行。**
...
| `dev-plan.md` / `integration-plan.md` | 各领域根目录 | 任务清单，带 ?/?/?? 状态 |
