...
├── deploy_planner.json   ← {"id":"abc123","type":"deploy-planner","updated":"..."}
├── deploy_infra.json     ← {"id":"def456","type":"deploy-infra","updated":"..."}
└── deploy_verifier.json  ← {"id":"ghi789","type":"deploy-verifier","updated":"..."}
```
...
1. resume DEPLOY_INFRA_ID，令其阅读验证报告并修正：
   ```
   Agent(resume: "{DEPLOY_INFRA_ID}", subagent_type: "deploy-infra",
     prompt: "请读取 {DEPLOY_ROOT}/deploy-verification-report.json 并修正所有问题。完成后简短确认。")
   ```
2. 重新启动 deploy-verifier 验证
3. 记录日志
