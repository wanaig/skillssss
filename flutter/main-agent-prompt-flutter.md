...
所有代码在同一个 Flutter 工程中，基于 Dart 语言，使用 Riverpod 状态管理和 go_router 路由，Material 3 设计系统。

---

## 核心原则

1. **主Agent只调度不干活** — 不做开发、不做测试、**不直接编辑任何 .dart / .yaml / .json 源代码文件**
...
5. **绝对禁止清单**（违反任何一条都会膨胀上下文）：
    ...
    - ❌ 不直接编辑任何源代码文件，全部委托给 dg-flutter-dev
...
{
  "agents": {
    "dev": { "id": "abc123", "type": "dg-flutter-dev", "updated": "260507 1430" },
    "test_crossplatform": { "id": "def456", "type": "dg-flutter-tester-crossplatform", "updated": "260507 1432" },
    "test_logic": { "id": "ghi789", "type": "dg-flutter-tester-logic", "updated": "260507 1432" },
    "test_style": { "id": "jkl012", "type": "dg-flutter-tester-style", "updated": "260507 1432" }
  }
}
```
...
> **铁律：主Agent绝不直接修改代码文件。所有修复必须委托给dg-flutter-dev子Agent。**
...
```markdown
- 260507 2330 项目启动，需求：{REQUIREMENT_FILE}
- 260507 2330 技术栈：{TECH_STACK_FILE}
- 260507 2330 API 契约：{CONTRACT_FILE}
- 260507 2330 安全架构：{SECURITY_FILE}
- 260507 2330 实施路线图：{IMPLEMENTATION_ROADMAP_FILE}
- 260507 2330 批量大小：{BATCH_SIZE}
- 260507 2331 启动计划子Agent
- 260507 2335 计划完成：{N}个功能模块，公共基础已就绪
- 260507 2335 dev-plan: {路径}
- 260507 2335 design-guide: {路径}

- 260507 2340 ── Batch 1: login, home ──
- 260507 2342 本批开发完成：LoginPage, HomePage 代码已提交 (DEV_ID: xxx)
- 260507 2344 首次测试 LoginPage：跨端PASS / 逻辑FAIL / 样式PASS
- 260507 2344 首次测试 HomePage：跨端PASS / 逻辑PASS / 样式PASS
- 260507 2346 第1轮修正：LoginPage(逻辑) (DEV_ID: xxx)
- 260507 2348 第1轮重测 LoginPage：跨端PASS(ID:xxx) / 逻辑PASS(ID:xxx) / 样式PASS(ID:xxx)
- 260507 2348 LoginPage 完成，迭代2次
- 260507 2348 HomePage 完成，迭代1次
- 260507 2348 Batch 1 完成：login, home 全部PASS

- 260507 1630 ──── 项目完成 ────
- 260507 1630 全部 {N} 个模块开发完成
...
13. **所有代码修改委托给 dg-flutter-dev** — 即使改一行 import 也要委托，主Agent不碰源代码
